...
class Stemmer {
	...
	public String getStemmingWord(String word) {
		Stemmer s = new Stemmer();
		String returnword = "";
		char[] charector = word.toCharArray();
		
		for (int i = 0; i < charector.length; i++) {
			
			//if(Character.isLetter(charector[i]))
				s.add(charector[i]);
			//else 
				//returnword = word; break;
		}
		s.stem();
		returnword = s.toString();
		return returnword;
...
/*		connection
		...
		String s = st.getStemmingWord("connecting");
